...
OBJ_HXX_DIR	    ?= $(BUILD_DIR)/object_hxx
...
DEPENDS_HPP     := $(patsubst %.hpp,$(OBJ_HXX_DIR)/%.d,$(filter %.hpp,$(SOURCES)))
...
$(OBJ_HXX_DIR)/%.d: %.hpp
	...
	@set -e; rm -f $@; \
		$(COMPILE) -MM $(FLAGS) $< > $@.$$$$; \
		sed 's,$(notdir $*)\.o[ :]*,$(OBJ_HXX_DIR)/$*\.o $@ : ,g' < $@.$$$$ > $@; \
		rm -f $@.$$$$
...
#只清除.o文件
clean:
	@rm -rf $(OBJ_C_DIR) $(OBJ_CXX_DIR) $(OBJ_HXX_DIR)
	@echo "Clean OK ..."
